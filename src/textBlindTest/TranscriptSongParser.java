...
public class TranscriptSongParser {
	...
	protected static List<SongLine> ParseEnglish() throws IOException {
		...
		Pattern pattern = Pattern.compile("^*@.+?@$"); // Regex to get all Strings of type "{any text}@{SongLine
														// lyric}@"
		Matcher matcher;
		...
		while ((line = in.readLine()) != null) {
			...
			if (matcher.find()) {
				listeQuotesChecked.add(new SongLine("english", o[3].substring(1, o[3].length() - 1),
						Integer.parseInt(o[0].substring(1)), Integer.parseInt(o[1].substring(1))));
			}
	...
	protected static List<SongLine> fillList() throws IOException {
		...
		while ((line = in.readLine()) != null) {
			if (line.charAt(0) != '#') {
				...
				list.add(new SongLine(o[0], "english", o[1], Integer.parseInt(o[2].substring(1)),
						Integer.parseInt(o[3].substring(1))));
			}
		}
		in.close();
		return list;
