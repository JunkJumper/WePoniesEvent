...
public class Start {
	...
	private static class LocalGameManager {
		...
		public static void RandomQuote(SongLibrary library, int MAX) throws InterruptedException {
			...
			int getSizeOfLyric = library.getBibli().get(getRandomLyricIndex).getLyric().getL().size() - 1; // return the
																											// size of
																											// Lyrics
																											// String
			int lastIndexOfLyric = Start.alea(getSizeOfLyric); // return the last index of the lyric (ex 9)
			...
			while (currentRandomLyric < lastIndexOfLyric) {
				System.out
						.println(library.getBibli().get(getRandomLyricIndex).getLyric().getL().get(currentRandomLyric));
				currentRandomLyric++;
			}
			System.out.println("======================");
			Thread.sleep(8000);
		...
		public static void SongFrom(SongLibrary library, int season, int episode) throws InterruptedException {
			...
			for (int j = 0; j < library.getBibli().size(); j++) {
				if (library.getBibli().get(j).getEpisode() == episode
						&& library.getBibli().get(j).getSeason() == season) {
					i = j;
				}
			}
			if (i != -1) {
